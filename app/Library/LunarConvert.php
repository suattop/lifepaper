...
class LunarConvert
{
     ...
     * @var array
     */
    private static $JIE_QI = ['冬至', '小寒', '大寒', '立春', '雨水', '驚蟄', '春分', '清明', '穀雨', '立夏', '小滿', '芒種', '夏至', '小暑', '大暑', '立秋', '處暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪'];
    ...
    private function computeJieQi()
    {
        ...
        for ($i = 0, $j = count(LunarConvert::$JIE_QI); $i < $j; $i++) {
            $t = floatval(bcadd(bcadd($this->calJieQi($jd + $i * 15.2, $i * 15 - 90).'', SolarConvert::$J2000.''), bcdiv(8, 24)));
    ...
    private function gxc($t, $pos)
    {
        ...
        /** 近點 */
        $gxcp = [bcdiv(102.93735, $degreePerRad), bcdiv(1.71946, $degreePerRad), bcdiv(0.00046, $degreePerRad)];
        /** 太平黃經 */
        $gxcl = [bcdiv(280.4664567, $degreePerRad), bcdiv(36000.76982779, $degreePerRad), bcdiv(0.0003032028, $degreePerRad), bcdiv(bcdiv(1, 49931000), $degreePerRad), bcdiv(bcdiv(-1, 153000000), $degreePerRad)];
        /** 光行差常數 */
